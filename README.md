...
- [ ] 응모 제출 → 구글 시트에 행 추가됨 (503 이 아니라 성공)
- [ ] 같은 번호로 재제출 → 중복 차단됨
- [ ] 카카오톡에 링크를 보내 **썸네일이 뜨는지** (안 뜨면 `NEXT_PUBLIC_SITE_URL` 문제)
- [ ] STEP 0 버튼이 `goo.gle/kangwonchuncheon` 으로 가는지
- [ ] GA4 실시간에서 `page_view`, `signup_click` 이 잡히는지

## 배포 전 남은 일

콘텐츠·운영 항목이라 코드로 채울 수 없습니다.

- [x] ~~`public/samples/sample-lecture.pdf`~~ — 임시 샘플(3쪽) 생성 완료. `npm run sample-pdf` 로 재생성.
      실제 강의자료가 준비되면 같은 이름으로 덮어쓰세요 (저작권 문제 없는 것)
- [ ] **`app/privacy/page.tsx`의 `OPERATOR`** — 담당자 성명·이메일. 담당자 정보 없이 개인정보를 수집하면 안 됩니다
- [ ] **`apps-script/Code.gs`의 `SECRET`** — 긴 무작위 문자열로 교체 후 Vercel 환경변수와 일치시키기
- [ ] **Google AI Pro 학생 인증 실제 경로 확인** — 강원대 이메일로 직접 테스트
  (링크는 `https://goo.gle/kangwonchuncheon` 로 반영됨. UTM 을 붙이려면 `NEXT_PUBLIC_SIGNUP_URL` 로 덮어쓰기)
- [ ] **제미나이 UI 캡처** — UI가 자주 바뀌므로 캠페인 직전 주에 다시 확인
- [ ] **"대충 쓴 프롬프트 vs 잘 쓴 프롬프트" 결과 비교 스크린샷**

### 2차 (캠페인 중 추가 가능)

- [ ] 레시피 카드 `/recipes` — "시험 2주 전", "마감 3일 전" 등 상황별 툴 조합
- [ ] 심화 트랙 — Nano Banana 발표자료, 전공별, 팀플
- [ ] PC 분할화면 안내
...
유입이 에브리타임·단톡방이므로 **인앱 브라우저에서 깨지면 캠페인 전체가 깨집니다.**

- [ ] 에브리타임 인앱 브라우저 — localStorage 동작 / 새 탭 열기 / 앱 전환 후 진도 유지
- [ ] 카카오톡 인앱 브라우저 — 위와 동일
- [ ] iOS Safari, Android Chrome
- [ ] STEP 2에서 제미나이 앱을 열고 3분 뒤 돌아왔을 때 이어지는가
- [ ] 프롬프트 복사가 되는가 (인앱에서 막히면 폴백 안내가 뜨는지)
- [ ] 응모: 중복 제출 차단 / 동의 미체크 거부 / 잘못된 번호 거부 / 시트 기록 확인
- [ ] GA4 DebugView로 이벤트 발화 확인
- [ ] **파일럿**: 학생 5~10명에게 실제로 시켜서 완주 시간 측정. 15분을 넘으면 콘텐츠를 깎을 것
